...
public:
    ...
    enum Props {
        ...
        // Признак повторной доставки последнего сообщения. Без него защиту
        // от дублей в поштучном режиме построить не на чем: в пакетном
        // и событийном признак есть в JSON, а здесь его не было.
        ePropRedelivered,
        // Сколько раз платформа отказалась принять событие. В событийном
        // режиме это единственный способ узнать об отставании: GetLastError
        // там никто не читает.
        ePropEventsRejected,
        ePropLast
    };
