...
public:
    ...
    // Остановить доставку, сохранив канал: брокер перестаёт слать новое,
    // а уже полученное можно дочитать и подтвердить. Без этого при завершении
    // работы возвращается в очередь всё, что компонента успела получить
    // вперёд по prefetch, — и приходит повторно.
    void stopDelivery();
