...
public:
    ...
    // Желаемый интервал heartbeat, секунды:
    //   -1 — принять предложение брокера (по умолчанию),
    //    0 — выключить,
    //   >0 — своё значение, но не больше предложенного брокером.
    // Брокер разрывает соединение, от которого не было ни байта два интервала,
    // поэтому кадры отправляет сам транспорт: простаивающее фоновое задание
    // обязано оставаться на связи.
    virtual void setHeartbeat(int seconds) = 0;
